...
string *decode(bitset<32> instruction) {
	...
	if (op_code == "000000") {	//R-type
	...
	else if (op_code == LW || op_code == SW || op_code == BEQ || op_code == ADDIU) { //I-type LW SW BEQ
		...
		decode_J_result[0] = op_code;
		decode_J_result[1] = bitset<26>(instruction.to_string().substr(6, 31)).to_string();
	}
	
	return decode_J_result;
}
...
int main()
{
	...
	while (1)
	{
		...
		// decode(Read RF)
			...
			if (p[5] == "000000") {	//R-type u-type; p = {rs, rt, rd, funct, funct_0_2, op_code}  Debug completed!
			...
			}else if (p[3] == LW) { //I-type:LW, p = {imm, rt, rs, op_code}]   Debug completed!
				...
				if (bitset<16>(p[0]).test(15)) { //sign = 1,negative
					LW_imm = bitset<32>("1111111111111111" + p[0]);
				}
				else {
					LW_imm = bitset<32>(p[0]);
				}
				bitset<32> addr = bitset<32> (rs_data_at_addr.to_ulong() + LW_imm.to_ulong()); // addr = rs_addr + imm
			...
			else if (p[3] == SW) { //I-type:SW, p = {imm, rt, rs, op_code}
				cout << "imm = " << p[0] << endl;
				cout << "rt = " << p[1] << endl;
				cout << "rs = " << p[2] << endl;
				cout << "op_code = " << p[3] << endl;
				bitset<32> SW_imm;
				myRF.ReadWrite(bitset<5>(p[2]), bitset<5>(p[1]), NULL, NULL, bitset<1>("0")); //read rs->data1, rt->data2
				bitset<32> rs_addr = myRF.ReadData1;
				if (bitset<16>(p[0]).test(15)) { //sign = 1,negative
					SW_imm = bitset<32>("1111111111111111" + p[0]);
				}
				else {
					SW_imm = bitset<32>(p[0]);
				}
				bitset<32> addr = bitset<32>(rs_addr.to_ulong() + SW_imm.to_ulong()); // addr = rs_addr + imm
				...
				myRF.ReadWrite(bitset<5>(p[2]), bitset<5>(p[1]), NULL, NULL, bitset<1>("0")); //read rs->data1, rt->data2
				if (myRF.ReadData1.to_string() == myRF.ReadData1.to_string()) {
					bitset<30> BEQ_imm;
					if (bitset<16>(p[0]).test(15)) { //sign = 1,negative
						BEQ_imm = bitset<30>("11111111111111" + p[0]);
					}
					else {
						BEQ_imm = bitset<30>(p[0]);
					}
					bitset<32> beq_addr = bitset<32> (BEQ_imm.to_string() + "00"); //branchAddress
					pc = bitset<32>(pc.to_ulong() + beq_addr.to_ulong());
				}
			}
			else if (p[3] == ADDIU) {
				cout << "imm = " << p[0] << endl;
				...
				bitset<32> ADDIU_imm;
				if (bitset<16> (p[0]).test(15)) { //sign = 1,negative
					ADDIU_imm = bitset<32>("1111111111111111" + p[0]);
				}
				else {
					ADDIU_imm = bitset<32>(p[0]);
				}
				cout << "ADDIU_imm = " << ADDIU_imm << endl;
				bitset<32> ADDIU_result = bitset<32> (rs.to_ulong() + ADDIU_imm.to_ulong());
				myRF.ReadWrite(NULL, NULL, bitset<5>(p[1]), ADDIU_result, bitset<1>("1"));
			}
			else if (p[0] == J) {
				cout << "imm = " << p[1] << endl;
				cout << "op_code = " << p[0] << endl;
				//cin.get();
				j_flag = true;
				pc = bitset<32>(pc.to_ulong() + 4);
				string jumpaddr = pc.to_string().substr(0, 3) + p[1] + "00";
				cout << "jmpaddr = " << jumpaddr << endl;
				pc = bitset<32>(jumpaddr);
				cout << "pc = " << pc << endl;
				cin.get();
				//continue;//may result a loss of rf record?
			}
		}
		// Execute
		...
		if (!j_flag) pc = bitset<32> (pc.to_ulong() + 4);
